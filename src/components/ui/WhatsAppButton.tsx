...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    // No desktop, aparece imediatamente após 10% de scroll
    // No mobile, aparece após 40% de scroll
    const checkVisibility = () => {
      const isMobile = window.innerWidth < 768;
      const threshold = isMobile ? 0.4 : 0.1;
      const scrollPercent =
        window.scrollY / (document.body.scrollHeight - window.innerHeight);
      setVisible(scrollPercent > threshold);
    };

    // Mostrar após 1s no desktop independentemente do scroll
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          >
            <WhatsAppSvg className="h-7 w-7 md:h-8 md:w-8" />
          </motion.div>
        </motion.a>
      )}
    </AnimatePresence>
